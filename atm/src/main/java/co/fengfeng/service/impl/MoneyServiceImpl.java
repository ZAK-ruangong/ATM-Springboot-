...
import co.fengfeng.common.util.DateUtil;
...
public class MoneyServiceImpl implements MoneyService {
    ...
    public AjaxRes getUserMoney(String cardId) {
        ...
        try {
            ...
            CardInfo cardInfo1 = cardInfoMapper.selectOne(queryWrapper);
    ...
    public AjaxRes updateMoneyById(String cardId, BigDecimal money, HttpSession session) {
        ...
        try {
            ...
            if (i < 0) {
                ...
                cardInfoMapper.update(cardInfo1,queryWrapper);
                /**************保存交易记录*****************/
    ...
    public AjaxRes getUserCard(String cardId, HttpSession session) {
        ...
        try {
            ...
            if (StringUtils.isEmpty(cardInfo1)) {
                ...
                UserInfo userInfo = userInfoMapper.selectById(cardInfo1.getCustomerId());
                ajaxRes.setRes("success");
    ...
    public AjaxRes transMoneyByCard(String userCard, String transCard, BigDecimal money,HttpSession session) {
        ...
        try {
            ...
            QueryWrapper<CardInfo> queryWrapper = new QueryWrapper<>();
            queryWrapper.eq("card_id",userCard);
            CardInfo cardInfo = cardInfoMapper.selectOne(queryWrapper);

            QueryWrapper<CardInfo> queryWrapper2 = new QueryWrapper<>();
            queryWrapper2.eq("card_id",transCard);
            CardInfo cardInfo2 = cardInfoMapper.selectOne(queryWrapper2);


            //CardInfo cardInfo = cardInfoMapper.selectById(userCard);
            ...
            if (i >= 0) {
                ...
                System.out.println("**************************************************"+cardInfo);
                cardInfoMapper.update(cardInfo,queryWrapper);
                System.out.println("************************"+cardInfo);
                //对方金额增加
                //CardInfo cardInfo2 = cardInfoMapper.selectById(transCard);
                cardInfo2.setMoney(cardInfo2.getMoney().add(money));
                //cardInfoMapper.updateById(cardInfo2);
                cardInfoMapper.update(cardInfo2,queryWrapper2);
